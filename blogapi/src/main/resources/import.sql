--this script initiates db for h2 db (used in test profile)
insert into user (id, account_status, email, first_name, last_name) values (null, 'CONFIRMED', 'john@domain.com', 'John', 'Steward')
insert into user (id, account_status, email, first_name) values (null, 'NEW', 'brian@domain.com', 'Brian')
insert into user (id, account_status, email, first_name, last_name) values (null, 'CONFIRMED', 'martha@domain.com', 'Martha', 'Johnson')
insert into user (id, account_status, email, first_name, last_name) values (null, 'REMOVED', 'robert@domain.com', 'Robert', 'Mist')
insert into blog_post values (null, 'test', 1)
insert into blog_post values (null, 'test', 2)
insert into blog_post values (null, 'test', 3)
insert into blog_post values (null, 'test', 4)